...
Das Diagramm \ref{fig:usecasediagram} zeigt eine Übersicht der zu implementierenden
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lX}
		...
		\tablebody
			...
			\tabularnewline
			WG-Administrator &
			Der WG-Administrator ist eine Erweiterung des Akteurs Bewohner. Der Ersteller einer WG wird automatisch zu deren Administrator. Diese Rolle kann an Bewohner weitergegeben werden (siehe \nameref{subsec:uc4} und \nameref{subsec:uc10}).
			\tabularnewline
			Facebook &
			Facebook ist die Schnittstelle zur Social Media Plattform. Dies ermöglicht ein vereinfachtes Login sowie das Teilen von Daten und Links.
			\tabularnewline
		\tableend
	\end{tabularx}
	\caption{Akteure}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lX}
		\tablebody
			...
			\tabularnewline
			\textit{Story} &
			Der Bewohner startet Roomies. Das System zeigt das Anmeldeformular. Der Benutzer meldet sich mittels seines Facebook-Logins an. Das System überprüft die Daten. Sind sie gültig wird der Benutzer auf die WG-Startseite weitergeleitet.
			\tabularnewline
		\tableend
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lX}
		\tablebody
			...
			\tabularnewline
			\textit{Story} &
			Wählt ein Bewohner die Option ``WG verlassen'', wird er vom System aufgefordert dies zu Bestätigen. Nach der Bestätigung setzt das System den Bewohner auf inaktiv und leitet den ``Ex''-Bewohner auf eine ``Aufwiedersehen-Seite'' weiter. \newline
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lX}
		\tablebody
			...
			\tabularnewline
			\textit{Story} &
			Der Bewohner wählt die Aufgabe welche bearbeitet werden soll. Parallel zum ``\nameref{subsec:uc5}'' wird ein Formular mit den bereits vorhandenen Daten der Aufgabe dargestellt. Der Bewohner ändert die Daten. Das System überprüft diese und leitet dann den Bewohner auf die Aufgabenliste weiter.
			\tabularnewline
		\tableend
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lX}
		\tablebody
			...
			\tabularnewline
			\textit{Story} &
			In der Aufgabenliste kann ein Bewohner eine Aufgabe als erledigt markieren.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lX}
		\tablebody
			...
			\tabularnewline
			\textit{Story} &
			Der WG-Administrator hat die Möglichkeit eine WG aufzulösen. Wird diese Option gewählt, so wird vom WG-Administrator verlangt, dies zu bestätigen. Danach setzt das System alle Bewohner der WG sowie die WG selbst als inaktiv. Der WG-Administrator wird auf die WG-Erstellen Seite weitergeleitet.
			\tabularnewline
		\tableend
	\end{tabularx}
