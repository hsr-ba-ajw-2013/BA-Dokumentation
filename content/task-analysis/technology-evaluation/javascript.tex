...
\textit{derby} &
	...
	\tabularnewline
\textit{Geddy} &
	Geddy \cite{Geddy} ist ein weitere node.js Kandidat, welcher sich Ruby on Rails zum Vorbild nimmt. Dementsprechend vergleichbar ist der Funktionsumfang mit Tower.js. Einer der auffälligeren Unterschiede ist jedoch, dass Geddy auf CoffeeScript vollends verzichtet.
	\tabularnewline
\textit{Sails} &
	Sails \cite{sails} ist der jüngste Frameworkkandidat für JavaScript. Die Grundkonzepte von Ruby on Rails werden mit um einen interessanten ``\gls{Realtime}''-Aspekt mittels \glspl{Websocket} erweitert. So kann jede Ressource über eine einfache REST-Schnittstelle als auch über eine \gls{Websocket}-Verbindung angesprochen werden. Damit erleichtert sich das asynchrone resp. servergesteuerte Aktualisieren des Frontends. Zusätzlich beinhaltet Sails bereits eine \gls{ORM} Bibliothek.
	\tabularnewline
\tableend
\end{tabularx}
