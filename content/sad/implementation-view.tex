...
\emph{Barefoot} \cite{Barefoot} ermöglicht die Verwendung desselben \emph{Backbone.js} Quelltexts sowohl auf dem Server als auch auf dem Client. Dies wird u.A. durch gezieltes Einsetzen von Codefragmenten mittels \gls{Mixin}s erreicht.

Beim initialisieren des Frameworks wird einmalig festgestellt, in welcher Umgebung (siehe Quelltext \ref{lst:BarefootDetermineRuntimeEnvironment} der Code ausgeführt werden soll.

\begin{lstlisting}[language=JavaScript, firstnumber=68, caption={Ermittlung der Runtime-Umgebung sowie laden der abhängigen Mixins \cite{BarefootDetermineRuntimeEnvironment}}, label={lst:BarefootDetermineRuntimeEnvironment}]
