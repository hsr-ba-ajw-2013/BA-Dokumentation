\section{Entity-Relationship Diagramm}

\begin{figure}[ht!]
	\begin{tikzpicture}
		% row by row
		\umlclass[x=20,y=0]{Community}{
			id : Integer \\
			name : String \\
			createdAt : Date \\
			updatedAt : Date
		}{
			PRIMARY(id)
		}

		\umlclass[x=12,y=-6]{User}{
			id: Integer \\
			facebookId: Integer \\
			communityId: Integer \\
			name: String \\
			createdAt: Date \\
			updatedAt: Date \\
			active: Boolean \\
			isAdmin: Boolean
		}{
			PRIMARY(id) \\
			UNIQUE(facebookId)
		}

		\umlclass[x=12,y=-12]{Achievement}{
			id: Integer \\
			type: Enum \\
			userId: Integer \\
			createdAt: Date
		}{
			PRIMARY(id)
		}

		\umlclass[x=20,y=-6]{Task}{
			id: Integer \\
			communityId: Integer \\
			name: String \\
			description: Text \\
			createdAt: Date \\
			updatedAt: Date \\
			creatorId: User \\[1mm]
			fulfillorId: User
		}{
			PRIMARY(id)
		}

		% relation
		\umlassoc[mult1=0..*,mult2=1]{Achievement}{User}
		\umlassoc[mult1=1,mult2=0..*]{Community}{Task}
		\umlassoc[geometry=|-, mult1=1..*,mult2=1, pos2=1.9]{User}{Community}
		% creator
		\umlassoc[geometry=-|-, arm1=3, anchor1=40, anchor2=-28, pos2=2.0, mult2=1..*]{User}{Task}
		% fulfillor
		\umlassoc[geometry=-|-, anchor1=40, anchor2=-38, pos2=2.25, mult1=1,mult2=0..*]{User}{Task}
	\end{tikzpicture}

	\caption{Entity-Relationship Diagramm}
\end{figure}